...
| Capture input system hardening | In progress | `Phase R7A` contract lock and QA harness are complete; `Phase R7B` now rewrites the live capture panel around an AppKit-owned sizing host, and selector state repair is locked in `docs/Capture-Suggested-Target-Selector-Repair-Plan.md` |
